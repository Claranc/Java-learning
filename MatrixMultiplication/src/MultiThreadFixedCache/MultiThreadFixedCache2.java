...
public class MultiThreadFixedCache2 implements Runnable {
    private Matrix2 m1;
    private Matrix2 m2;
    ...
    MultiThreadFixedCache2(Matrix2 m1, Matrix2 m2, Lock mu1, Queue<Pair<Integer, Integer>> curIndex, int batch, int[][] result) {
        this.m1 = m1;
        this.m2 = m2;
        this.getKLock = mu1;
    ...
    private MultiThreadFixedCache2(Matrix2 m1, Matrix2 m2, Lock mu1, Queue<Pair<Integer, Integer>> curIndex, int batch, int[][] result, int i, int j, int k) {
        this.m1 = m1;
        this.m2 = m2;
        this.getKLock = mu1;
        this.curIndex = curIndex;
        ...
        for (int i = 0; i < batch * batch; i += batch) {
            for (int j = 0; j < batch * batch; j += batch) {
                for (int k = 0; k < batch * batch; k += batch) {
                    ...
                    for (int x = 0; x < num; x++) {
                        threadArray[x] = new Thread(new MultiThreadFixedCache2(m1, m2, getKLock, curIndex, batch, result, i, j, k));
                        threadArray[x].start();
                    }
                    for (int x = 0; x < num; x++) {
                        threadArray[x].join();
